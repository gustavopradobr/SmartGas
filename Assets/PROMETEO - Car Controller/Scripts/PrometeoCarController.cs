...
public class PrometeoCarController : MonoBehaviour
{
    ...
    void Start()
    {
        if (TryGetComponent(out PrometeoHelper _helper))
            helper = _helper;
    ...
    private void CalculateSteeringFactor()
    {
        if (helper != null)
        {
            float factor = frontLeftCollider.steerAngle / maxSteeringAngle;
            helper.RotateSteeringWheel(factor);
        }
    }
    ...
    public void Handbrake(){
      ...
      DriftCarPS();
        ...
        float handbrakeTorque = brakeForce * 10;
